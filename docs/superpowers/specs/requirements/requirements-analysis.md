...
| **REQ-020** | **前后端日志系统完善** | **日志不完善，无统一格式和请求追踪** | **winston + requestLogger + Axios 拦截器** | **✅ 已实现** |
...
| AI 配置 | 环境变量（.env），~~`server/config/deepseek.xml`~~（已删除）|
...
| 维度 | 实现位置 |
|------|----------|
| Token 中间件 | `server/middleware/auth.js` — Bearer token 校验，公开路由白名单 |
| axios 拦截器 | `client/src/main.js` — 请求自动附加 token，401 触发重登录 |
| API 格式统一 | 全部 10 个 route 文件 + 10 个前端 .vue 文件 |
| 环境变量 | `server/.env` — 数据库/AI Key/邮件配置 |
| user 表统一 | 移除 `auth.js` 中 `hasUsername` 运行时检测逻辑，直接使用 `username` |
| - | `server/index.js` — dotenv 启动加载 |
...
### 验收标准
- [x] 后端日志通过 winston 统一输出，支持按日轮转和级别分离
- [x] 每个 HTTP 请求自动记录 method、url、status、duration
- [x] 全局错误处理中间件捕获所有未处理异常并记录
- [x] 前端所有 API 请求通过 Axios 拦截器自动记录耗时和状态
- [x] deepseek.js 中的调试日志已清理
- [x] 日志规范文档同步更新
...
| 维度 | 实现位置 |
|------|----------|
| winston 封装 | `server/services/logger.js` — console + 按日轮转 error/combined |
| 请求日志中间件 | `server/middleware/requestLogger.js` — method/url/status/duration |
| 全局错误处理 | `server/index.js` — 路由之后、DB 初始化之前 |
| 前端 API 日志 | `client/src/main.js` — Axios 拦截器 |
| 调试日志清理 | `server/services/deepseek.js` — 删除 5 处泄露日志 |
| 规范文档 | `docs/superpowers/standards/logging-conventions.md` |
| deepseek 配置 | `server/config/deepseek.xml` 删除，迁移至环境变量 |

### Commits
- `c74bbf8` — chore: add winston dependencies
- `eb75177` — feat: add winston logger service with daily rotate
- `c615cc4` — feat: add request logging middleware
- `8b0b4f5` — feat: integrate request logger, global error handler, structured startup logs
- `e345ee7` — fix: remove debug console.log from deepseek service
- `e64735c` — feat: add axios request/response interceptors for API logging
- `ce395d9` — docs: update logging conventions for winston and frontend interceptors
- `2e44c0c` — fix: address code review findings (orphaned xml, doc header, auth log visibility)
- `4c25fe1` — docs: archive REQ-020 logging system to specs/modules

### 设计链接
- 需求文档: `docs/superpowers/requests/2026-07-14_REQ-020_logging-system.md`
- 实现计划: `docs/superpowers/plans/2026-07-14-logging-system-plan.md`
- 模块设计: `docs/superpowers/specs/modules/logging-module.md`
